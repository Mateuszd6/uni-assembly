...
void easyppm_gamma_correct(PPM* ppm, float gamma) {
    ...
    for (y = 0; y < ppm->height; y++) {
        for (x = 0; x < ppm->width; x++) {
            ...
            r = (PPMBYTE)(powf(c.r / (float)EASYPPM_MAX_CHANNEL_VALUE, exp_) * 255);
            g = (PPMBYTE)(powf(c.g / (float)EASYPPM_MAX_CHANNEL_VALUE, exp_) * 255);
            b = (PPMBYTE)(powf(c.b / (float)EASYPPM_MAX_CHANNEL_VALUE, exp_) * 255);
